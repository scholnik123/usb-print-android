...
Responsive layout uses one constraint-based `AdaptiveLayoutPolicy`, so live split-screen/foldable resizing is classified from the available Compose width rather than a device model or physical pixel count. Widths below 600 dp are `COMPACT`, 600–839 dp are `MEDIUM`, and 840 dp or wider are `EXPANDED`. Compact and medium windows use a single scrollable column. Expanded windows use two balanced panes for source selection and print actions, but automatically return to one column at font scale 1.5 or greater. The main scroll surface is centered and limited to 840 dp on wider windows; compact or narrow-pane printer actions stack vertically. `Scaffold` owns safe-drawing insets, while consumed content insets plus IME padding keep the scrollable controls clear of system bars, cutouts, and the keyboard. Compile-time Compose previews cover 320 dp, short landscape, medium, expanded, dark, and font-scale 2.0 configurations.

Domain, selected document/printer, settings, and active print state remain outside Activity composition in the application container and `MainViewModel`, so an Activity recreation does not start a second job. The launch/share intent is consumed only for the initial Activity instance; `onNewIntent` handles later documents. Small UI visibility/draft state that matters across rotation uses `rememberSaveable`.
